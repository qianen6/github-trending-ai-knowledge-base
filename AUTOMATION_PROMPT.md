...
必须完成：
1. 先运行 `python scripts/collect_trending.py --root . --date YYYY-MM-DD --evidence`，确定性采集 Today/This week/This month × Global/Python/TypeScript/JavaScript/Jupyter Notebook/Go/Rust 共21页，Spoken Language=Any；读取活动工作区中的 `proof/run-YYYY-MM-DD/collection.json`，保留页面哈希、缓存和明确失败记录。若输出 `COLLECT PARTIAL`，只重试失败页面或证据，不得用猜测补齐。
...
5. 先写活动工作区中的 proof/run-YYYY-MM-DD/incoming.candidate.json，执行 validate-cards；通过后才原子写入 incoming/YYYY-MM-DD.json 并 ingest。新安装的活动工作区为 `workspace/`。
6. ingest必须生成结构化DailyEdition和publication manifest。前端日榜/周榜/月榜及其项目并集以DailyEdition为唯一事实；只为这些项目准备中文 README：中文官方README原样复制，英文官方README完整忠实翻译，不摘要、不重组、不省略实质内容。
